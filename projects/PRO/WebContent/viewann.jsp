		...
		<div class="container">
			<div class="row d-flex align-items-center justify-content-center">
				<div class="about-content col-lg-12">
					...
					<p class="text-white link-nav"><a href="index.jsp">Home </a> <span class="lnr lnr-arrow-right"></span>  <a href="ann.jsp">공지사항</a></p>
				</div>
			</div>
		</div>
	</section>
...
<br><br><br>
	...
	<div class="container">
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					<tr>
						...
						<td colspan="2"><%= ann.getAnnTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
					</tr>
					<tr>
						<td>작성자</td>	
						<td colspan="2"><%= ann.getUserID() %></td>
					</tr>
					<tr>
						<td>작성일</td>	
						<td colspan="2"><%= ann.getAnnDate().substring(0, 11) + ann.getAnnDate().substring(11, 13) + "시"
						+ ann.getAnnDate().substring(14, 16) + "분"%></td>
					</tr>
					<tr>
						<td>내용</td>	
						<td colspan="2" style="min-height: 200px; text-align: left;"><%= ann.getAnnContent().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
					</tr>
					<tr>
						<td>첨부파일</td>	
						<td>
							<%
								...
								String directory = "C:\\Users\\mhj96\\Desktop\\PDA-Web\\annfile\\"+annID;/*application.getRealPath("/upload");*/
								String files[] = new File(directory).list();
	
								for(String file : files) {
									out.write("<a href=\"" + request.getContextPath() + "/downloadannAction?file=" +
			...
			<a href = "ann.jsp" class="btn btn-primary">목록</a>
			<%
				if(userID != null && userID.equals(ann.getUserID())){
			%>
				<a href="updateann.jsp?annID=<%= annID %>" class="btn btn-primary">수정</a>
				<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="deleteannAction.jsp?annID=<%= annID %>" class="btn btn-primary">삭제</a>
			<%					
				}
			%>
		</div>
...
<br><br>
	...
	<div class="container">
		<div class="row">
			<form method="post" action="writeannReplyAction.jsp">
				<table class="table table-striped"
					style="text-align: center; border: 1px solid #dddddd; width: 1125px">
					<tbody>
						<tr>
							<td><input type="hidden" name="ident" value=<%= annID%>></td>
						...
						ArrayList<Reply> list = replyDAO.getList(annID);
