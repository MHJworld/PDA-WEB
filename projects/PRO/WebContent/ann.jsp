		...
		<div class="container">
			<div class="row d-flex align-items-center justify-content-center">
				<div class="about-content col-lg-12">
					...
					<p class="text-white link-nav"><a href="index.html">Home</a>  <span class="lnr lnr-arrow-right"></span>  <a href="ann.jsp">공지사항</a></p>
				</div>
			</div>
		</div>
	</section>
	...
	<div class="container">
		<div class = "row">
			<table class="table table-striped" style="text-align:center; border:1px solid #dddddd"> 
				...
				<tbody>
					<%
						...
						ArrayList<Ann> list = annDAO.getList(pageNumber);
						for(int i = 0; i < list.size(); i++) {
					%>
					<tr>
						<td><%= list.get(i).getAnnID() %></td>
						<td><a class="black-s" href="viewann.jsp?annID=<%= list.get(i).getAnnID() %>"><%= list.get(i).getAnnTitle() %></a></td>
					 	<td><%= list.get(i).getUserID() %></td>
						<td><%= list.get(i).getAnnDate().substring(0, 11) + list.get(i).getAnnDate().substring(11, 13) + "시" + list.get(i).getAnnDate().substring(14, 16) + "분" %></td>
					</tr>
					<%
						}
					%>
			...
			<%
				if (pageNumber != 1) {
			%>
				<a href="ann.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arrow-left">이전</a>
			<%
				}
				if (annDAO.nextPage(pageNumber + 1)) {
			%>
				<a href="ann.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arrow-left">다음</a>
			<%
				}
			%>
				<!-- ID Check -->
			...
			%>
				<a href="writeann.jsp" class="btn btn-primary pull-right">글쓰기</a>
			<%
				} else if (session.getAttribute("userID") != null){
			%>
				<button class="btn btn-primary pull-right"
					onclick="if(confirm('관리자가 아닙니다'))location.href='ann.jsp';"
