	...
	<div class="container">
		<div class = "row">
			<table class="table table-striped" style="text-align:center; border:1px solid #dddddd"> 
				...
				<tbody>
					<%
						...
						ArrayList<Pda> list = pdaDAO.getList(pageNumber);
						for(int i = 0; i < list.size(); i++) {
					%>
					<tr>
						<td><%= list.get(i).getPdaID() %></td>
						<td><a class="black-s" href="view.jsp?pdaID=<%= list.get(i).getPdaID() %>"><%= list.get(i).getPdaTitle() %></a></td>
					 	<td><%= list.get(i).getUserID() %></td>
						<td><%= list.get(i).getPdaDate().substring(0, 11) + list.get(i).getPdaDate().substring(11, 13) + "시" + list.get(i).getPdaDate().substring(14, 16) + "분" %></td>
					</tr>
					<%
						}
					%>
			...
			<%
				if (pageNumber != 1) {
			%>
				<a href="pda.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arrow-left">이전</a>
			<%
				}
				if (pdaDAO.nextPage(pageNumber + 1)) {
			%>
				<a href="pda.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arrow-left">다음</a>
			<%
				}
			%>
				<!-- 회원만넘어가도록 -->
