		...
		<div class="container">
			<div class="row d-flex align-items-center justify-content-center">
				<div class="about-content col-lg-12">
					...
					<p class="text-white link-nav"><a href="index.jsp">Home </a>  <span class="lnr lnr-arrow-right"></span>  <a href=""> Community</a><span class="lnr lnr-arrow-right"></span>  <a href="pda.jsp">자유게시판</a></p>
				</div>
			</div>
		</div>
	</section>
...
<br><br><br>	
	...
	<div class="container">
		<div class="row">
			<table class="table table-striped"
				...
				<tbody>
					<tr>
						...
						<td colspan="2"><%= pda.getPdaTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
					</tr>
					<tr>
						<td>작성자</td>	
						<td colspan="2"><%= pda.getUserID() %></td>
					</tr>
					<tr>
						<td>작성일</td>	
						<td colspan="2"><%= pda.getPdaDate().substring(0, 11) + pda.getPdaDate().substring(11, 13) + "시"
						+ pda.getPdaDate().substring(14, 16) + "분"%></td>
					</tr>
					<tr>
						<td>내용</td>	
						<td colspan="2" style="min-height: 200px; text-align: left;"><%= pda.getPdaContent().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
					</tr>
					<tr>
						<td>첨부파일</td>	
						<td>
							<%
								...
								String directory = "C:\\Users\\mhj96\\Desktop\\PDA-Web\\pdafile\\"+pdaID;/*application.getRealPath("/upload");*/
								String files[] = new File(directory).list();
	
								for(String file : files) {
									out.write("<a href=\"" + request.getContextPath() + "/downloadpdaAction?file=" +
			...
			<a href = "pda.jsp" class="btn btn-primary">목록</a>
			<%
				if(userID != null && userID.equals(pda.getUserID())){
			%>
				<a href="update.jsp?pdaID=<%= pdaID %>" class="btn btn-primary">수정</a>
				<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="deleteAction.jsp?pdaID=<%= pdaID %>" class="btn btn-primary">삭제</a>
			<%					
				}
			%>
		</div>
		...
		<div class="row">
			<form method="post" action="writeReplyAction.jsp">
				<table class="table table-striped"
					...
					<tbody>
						<tr>
							<td><input type="hidden" name="ident" value=<%= pdaID%>></td>
						...
						ArrayList<Reply> list = replyDAO.getList(pdaID);
